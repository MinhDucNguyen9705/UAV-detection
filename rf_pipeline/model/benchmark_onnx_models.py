...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Benchmark ONNX detector and detector+classifier pipelines.")
    parser.add_argument("--models-dir", type=Path, required=True, help="Folder containing .onnx models and optional .onnx.json metadata.")
    parser.add_argument("--images-dir", type=Path, default=None, help="Folder containing spectrogram/images used for benchmarking.")
    parser.add_argument(
        "--synthetic-images",
        type=int,
        default=0,
        help="Generate this many random benchmark images when --images-dir is not provided.",
    )
    ...
    parser.add_argument("--out", type=Path, default=Path("runs/onnx_benchmark"), help="Output directory for CSV/JSON reports.")
    ...
    parser.add_argument("--detector-two-pattern", default="", help="Optional glob/name fragment for 2-class detector ONNX files.")
    parser.add_argument("--detector-single-pattern", default="", help="Optional glob/name fragment for single-class detector ONNX files.")
    parser.add_argument("--classifier-pattern", default="", help="Optional glob/name fragment for classifier ONNX files.")
    parser.add_argument(
        "--classify-full-frame-if-empty",
        action="store_true",
        help="Run classifier on the full image when a single-class detector returns no boxes.",
    )
...
def main() -> int:
    ...
    with image_source(args) as images:
        ...
        for detector_path in model_groups.two_class_detectors:
            rows.append(
                run_guarded(
                    lambda p=detector_path: benchmark_detector(
                        p,
                        images=images,
                        args=args,
                        benchmark_name="detector_two_class",
                    ),
                    args.fail_fast,
        ...
        for detector_path in model_groups.single_class_detectors:
            for classifier_path in model_groups.classifiers:
                rows.append(
                    run_guarded(
                        lambda d=detector_path, c=classifier_path: benchmark_detector_classifier(
                            d,
                            c,
                            images=images,
                            args=args,
    ...
    print(f"Reports: {args.out / 'onnx_benchmark.csv'}")
    return 0
...
def benchmark_detector_classifier(
    detector_path: Path,
    ...
    classifier = ImageClassifier(classifier_path, device=args.device, backend="onnx")
...
def classify_predictions(predictions, classifier: ImageClassifier, classify_full_frame_if_empty: bool) -> int:
    ...
    for image_path, detections in predictions.items():
        if not detections and classify_full_frame_if_empty:
            xyxy = full_image_box(image_path)
            classifier.classify_crop(image_path, xyxy, fallback_name="signal", fallback_class_id=0)
            count += 1
            continue
        for detection in detections:
            classifier.classify_crop(
                image_path,
                detection.xyxy,
...
class image_source:
    ...
    def __enter__(self) -> list[Path]:
        ...
        self._temp_dir = tempfile.TemporaryDirectory(prefix="onnx_benchmark_images_")
        image_dir = Path(self._temp_dir.name)
...
def discover_models(models_dir: Path, two_pattern: str, single_pattern: str, classifier_pattern: str) -> ModelGroups:
    models = sorted(models_dir.rglob("*.onnx"))
    classifiers: list[Path] = []
...
def matches_filter(path: Path, pattern: str) -> bool:
    ...
    return path.match(pattern) or pattern.lower() in lowered


def infer_model_kind(path: Path) -> str:
    metadata = load_metadata(path)
    metadata_type = str(metadata.get("type", "")).lower()
    if "classifier" in metadata_type:
        return "classifier"
    if "detector" in metadata_type:
        class_count = metadata_class_count(metadata)
        return "single_detector" if class_count == 1 else "two_detector"
...
def metadata_class_count(metadata: dict) -> int:
...
def write_reports(rows: list[BenchmarkRow], out_dir: Path) -> None:
    csv_path = out_dir / "onnx_benchmark.csv"
    json_path = out_dir / "onnx_benchmark.json"
